...
const HobbyModal = (props) => {
    ...
    useEffect(() => {
        ...
        const fetchSkill = async () => {
            const res = await getAxiosRequest("hobbies/"+props.idTitle);
            setSkill(res.data);
            setIsLoading(false);
        }
        ...
        <Fragment>
            ...
            <ModalOverlay>
                <div className={classes.closeModal}>
                    <span className={classes.articleMode}><Link to={"/hobby/"+skill[0].idTitle} onClick={navigationToArticleHandler}><h6>Switch to article mode</h6></Link></span>
